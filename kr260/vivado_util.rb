...
class Vivado
  ...
  def set_write_hw_platform(name)
    @write_hw_platform = name
  end
  ...
  def _generate_build_tcl(dst)
    ...
    dst.puts("write_hw_platform -fixed -include_bit -force -file #{@write_hw_platform}") if @write_hw_platform != nil
    dst.puts("close_project")
    dst.puts("quit")
  end
